				...
				<div className="nav-bar">
					<div className="nav-wrapper">
						<ul>
							...
							<li className="pull-right"><Link onClick={() => {
								let keysToRemove = ["number_of_events_per_month", "league_id","dataOfLeagueWizard"];
								for (let key of keysToRemove) {
									localStorage.removeItem(key);
								}
								// localStorage.removeItem("number_of_events_per_month")
							}} to='/logout'>Logout<FontAwesomeIcon icon={faSignOutAlt} /></Link></li>
						</ul>
					</div>
				</div>
				);
